...
class MobileNetV2(nn.Module):
    def __init__(
        ...
        Args:
            ...
            block: Moudule specifying inverted residual building block for
            mobilenet
        ...
        if len(inverted_residual_setting) == 0 or len(inverted_residual_setting[0]) != 4:
            raise ValueError("inverted_residual_setting should be non-empty"
                    "or a 4-element list, got{}".format(interted_residual_setting))
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # This exists since TorchScript doesn't support inheritance, so
        # superclass method
        # (this one) needs to have a name other than 'forward' that can be
        # accessed in a subclass
        x = self.features(x)
        # Cannot use "squeeze" as batch-size can be 1 => use resahpe with
        # x.shape[0]
...
def mobilenetv2():
    ...
    #madds, params = torch.profiler.profile(model, inputs=(input,))
